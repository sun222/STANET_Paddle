...
def calibrate(model, dataload, batch_num, batch_size, num_workers=5):
...
def get_pruned_params(model):
    params = []
    skip_vars = []  # skip the first conv2d and last linear
        # skip_vars = ['linear_0.b_0',
        #              'conv2d_0.w_0']  # skip the first conv2d and last linear
    for sublayer in model.sublayers():
        for param in sublayer.parameters(include_sublayers=False):
            if isinstance(sublayer, paddle.nn.Conv2D) and sublayer._groups == 1 and param.name not in skip_vars:
...
class BaseModel:
    ...
    def get_sensitivity(self,
        ...
        sen_file='./prunemodel_pre/model.sensi.data'    
        if criterion == 'l1_norm':
            self.pruner = L1NormFilterPruner(self.net, inputs=inputs,sen_file='./prunemodel_pre/model.sensi.data')
        else:
            self.pruner = FPGMFilterPruner(self.net, inputs=inputs,sen_file='./prunemodel_pre/model.sensi.data')


        logging.info('Sensitivity analysis of model parameters starts...')
        # self.pruner.sen_file(
        #     eval_func=partial(_pruner_eval_fn, self, dataset, batch_size),
        #     sen_file=sen_file)
        logging.info(
            'Sensitivity analysis is complete. The result is saved at {}.'.
    ...
    def prune(self, pruned_flops, save_dir=None,eval_dataset=None,optimizer=None):
        """
        Args:
        ...
        metric = self.evaluate(eval_dataset, batch_size=1)
        print(metric)
    ...
    def prune_nosensi(self, pruned_flops, save_dir=None,eval_dataset=None,optimizer=None):
        """
        Args:
            pruned_flops(float): Ratio of FLOPs to be pruned.
            save_dir(None or str, optional): If None, the pruned model will not be saved.
        ...
        params = get_pruned_params(self.net)
        ratios = {}
        for param in params:
            ratios[param] = pruned_flops
        plan = self.pruner.prune_vars(ratios, [0])

        # logging.info("FLOPs after pruning: {}GFLOPs; pruned ratio: {}".format(
        # flops(self.net,  self.pruner.inputs) / 1000, plan.pruned_flops))
    ...
    def _prepare_post_qat(self,save_dir=None,eval_dataset=None):
        # ptq_config = {
        ...
        quant_model = ptq.quantize(self.net,fuse=True)

        print("Start calibrate...")

        self.eval_data_loader = self.build_data_loader(
            eval_dataset, batch_size=1, mode='train')
        calibrate(quant_model,  self.eval_data_loader,20,1, num_workers=0)
        ...
        # 3 save
        quant_output_dir = save_dir
        input_spec = paddle.static.InputSpec(
        shape=[1, 3, 256, 256], dtype='float32')
        ptq.save_quantized_model(quant_model, osp.join(quant_output_dir, 'model'),self._get_test_inputs([1,3,256,256]))
    ...
    def _export_inference_model(self, save_dir, image_shape=None):
        ...
        if self.status == 'Quantized':
            logging.info("The model for Quantized" )
